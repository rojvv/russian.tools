...
words = {
    r["id"]: r
    for r in read("words.csv")
    if r["type"] == "verb" and r["disabled"] == "0"
}
verbs = {
    r["word_id"]: r
    for r in read("verbs.csv")
...
with Path(sys.argv[2]).open("w", encoding="utf-8", newline="") as target:
    writer = csv.DictWriter(
        target,
        delimiter="\t",
        fieldnames=["bare", "accented", "translations_en", "aspect", *columns],
    )
    writer.writeheader()
    for word_id in sorted(
        verbs, key=lambda key: (int(words[key]["rank"] or 10**9), int(key))
    ):
        word = words[word_id]
        writer.writerow(
            {
