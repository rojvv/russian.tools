...
with open(sys.argv[1], encoding='utf-8', newline='') as source:
    for row in csv.DictReader(source, delimiter='\t'):
        if row['aspect'] not in ('perfective', 'imperfective', 'both'):
            continue
        forms = [accent(row['presfut_' + key]) for key in ('sg1', 'sg2', 'sg3', 'pl1', 'pl2', 'pl3')]
        ...
        result.append({
            ...
            'imperative': [accent(row['imperative_' + key]) for key in ('sg', 'pl')]
        })
Path('static/data/verbs.json').write_text(json.dumps(result, ensure_ascii=False, separators=(',', ':')), encoding='utf-8')
