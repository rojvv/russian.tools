export { normalizeWord as normalizeNoun, findWords as findNouns, suggestWords as suggestNouns } from './dictionary-lookup.ts';
...
export function createTable(noun: Noun) {
 return (['Singular', 'Plural'] as const).map((title) => ({
  title,
  rows: cases.map((label, index) => ({ label, form:
   (title === 'Singular' && noun.pluralOnly) || (title === 'Plural' && noun.singularOnly)
