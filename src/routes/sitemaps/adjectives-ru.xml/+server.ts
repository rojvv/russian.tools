...
export async function GET({ fetch }: { fetch: typeof globalThis.fetch }) {
  const [adjectives, nouns] = await Promise.all([getAdjectives(fetch), getNouns()]);
  const nounWords = new Set(nouns.map(({ bare }) => bare));
  return sitemap(dictionaryPaths("/decliner", adjectives.filter(({ bare }) => !nounWords.has(bare)), "ru"));
}
