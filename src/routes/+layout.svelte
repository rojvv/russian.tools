<script lang="ts">
	...
	$effect(() => { document.documentElement.lang = i18n.locale ?? 'en'; });
	function setLanguage(locale: Locale) {
		i18n.locale = locale;
		document.cookie = `language=${locale}; Path=/; Max-Age=${languageCookieMaxAge}; SameSite=Lax${location.protocol === 'https:' ? '; Secure' : ''}`;
	}
	const toolTitle = $derived(page.route.id === '/stress' ? i18n.t('stressTitle')
		: page.route.id === '/conjugator' ? i18n.t('verbTitle') : '');
</script>

<svelte:head>
	<link rel="preload" href="/fonts/OpenSans.ttf" as="font" type="font/ttf" crossorigin="anonymous" />
	<link rel="icon" href={favicon} />
</svelte:head>

<main>
	<header><h1><a href="/">russian.tools</a> {#if toolTitle}<span>{toolTitle}</span>{/if}</h1>
		<nav aria-label={i18n.t('language')}>
			<button lang="en" aria-pressed={i18n.locale === 'en'} onclick={() => setLanguage('en')}>English</button>
			<span aria-hidden="true">/</span>
			<button lang="ru" aria-pressed={i18n.locale === 'ru'} onclick={() => setLanguage('ru')}>Русский</button>
		</nav>
	</header>
	{@render children()}
</main>
