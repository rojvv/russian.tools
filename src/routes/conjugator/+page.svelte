<script lang="ts">
	...
	import { createTable, suggestVerbs, normalizeVerb, type Section, type Verb } from '$lib/conjugation';
	...
	let aspect = $state<'imperfective' | 'perfective'>(initial.matches[0]?.aspect === 'perfective' ? 'perfective' : 'imperfective');
	let sections = $state<Section[]>(initial.matches[0] ? createTable(initial.matches[0], initial.matches[0].aspect === 'perfective' ? 'perfective' : 'imperfective') : []);
	const finiteSections = $derived(sections.filter((section) => section.title === 'Present' || section.title === 'Future'));
	const otherSections = $derived(sections.filter((section) => section.title !== 'Present' && section.title !== 'Future'));
	...
	const suggested = $derived(verb && normalizeVerb(query).replaceAll('ё', 'е') !== normalizeVerb(verb.bare).replaceAll('ё', 'е'));
	...
	async function lookup() {
		...
		try {
			if (!dataset) {
				...
				}).catch((error) => { dictionaryRequest = undefined; throw error; });
...
<div class="lookup"> 
	<input id="verb" name="verb" lang="ru" bind:this={input} bind:value={query} oninput={handleInput} onchange={handleInput} placeholder={i18n.t('infinitive')} aria-label={i18n.t('infinitive')} spellcheck="false" autocapitalize="off" maxlength="40" />
 </div>
<p class="status" role="status">{loading ? i18n.t('lookingUp') : message ? i18n.t(message) : ''}</p>

{#if verb}
	{#if suggested}<p class="match">{i18n.t('closestMatch')} “{query.trim()}”: <strong lang="ru">{verb.infinitive}</strong></p>{/if}
	{#if matches.length > 1}
		<label for="meaning">{i18n.t('chooseEntry')}</label>
		<select id="meaning" value={selected} onchange={(event) => choose(Number(event.currentTarget.value))}>
			{#each matches as entry, i}<option value={i}>{entry.infinitive} — {i18n.t(entry.aspect)} — {entry.meaning}</option>{/each}
		</select>
	{/if}
	<h3 lang="ru">{verb.infinitive}</h3>
	<p class="meaning" lang="en">{#if i18n.locale === 'ru'}<span lang="ru">{i18n.t('meaningEnglish')}:</span>{' '}{/if}{verb.meaning}</p>
	{#if verb.aspect === 'both'}
		<label for="aspect">{i18n.t('bothAspectPrompt')}</label>
		<select id="aspect" bind:value={aspect} onchange={() => { sections = createTable(verb!, aspect); message = ''; }}>
			<option value="imperfective">{i18n.t('imperfectiveUse')}</option>
			<option value="perfective">{i18n.t('perfectiveUse')}</option>
		</select>
	{:else}
		<p class="hint">{i18n.t(aspect === 'perfective' ? 'perfectiveNote' : 'imperfective')}</p>
	{/if}
	<div class="conjugations">
			{#each finiteSections as section}
				<section aria-labelledby={`paired-${section.title.toLowerCase()}`}>
					<h4 id={`paired-${section.title.toLowerCase()}`}>{i18n.t(section.title as MessageKey)}</h4>
					<div class="paired">
						{#each (['Singular', 'Plural'] as const) as number, group}
							<div><h5>{i18n.t(number)}</h5><ul lang="ru">{#each section.rows.slice(group * 3, group * 3 + 3) as row}<li><span class="pronoun">{row.label}</span> {row.form || '—'}</li>{/each}</ul></div>
						{/each}
					</div>
				</section>
			{/each}
			{#each otherSections as section}
				<section aria-labelledby={`paired-${section.title.toLowerCase()}`}>
					<h4 id={`paired-${section.title.toLowerCase()}`}>{i18n.t(section.title as MessageKey)}</h4>
					<div class="paired">
						<div><h5>{i18n.t(section.title === 'Past' ? 'Singular' : 'singularInformal')}</h5><ul lang="ru">{#each section.rows.slice(0, -1) as row}<li><span class="pronoun">{shortLabel(row.label)}</span> {row.form || '—'}</li>{/each}</ul></div>
						<div><h5>{i18n.t(section.title === 'Past' ? 'Plural' : 'pluralFormal')}</h5><ul lang="ru">{#each section.rows.slice(-1) as row}<li><span class="pronoun">{shortLabel(row.label)}</span> {row.form || '—'}</li>{/each}</ul></div>
					</div>
				</section>
			{/each}
	</div>
