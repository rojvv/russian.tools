...
small entry files generated by `pnpm build:index` (also checked by `pnpm dev` and
`pnpm build`). Generation is skipped when the source dictionaries and indexing
code are unchanged and all generated files exist. Each dictionary index is
rebuilt independently; fresh checkouts generate both indexes once.
Exact Cyrillic and Latin forms, ё folding, homonyms, and prefix
